...
public class MyBatisSafeServlet extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        String username = req.getParameter("username");
        List<MyBatisUserRecord> users = MyBatisBootstrap.selectByUsernameSafe(username);

        resp.setContentType("text/html;charset=UTF-8");
        PrintWriter out = resp.getWriter();
        out.println("<html><body>");
        out.println("<h1>MyBatis 安全示例：使用 #{}</h1>");
        ...
        out.println("<p style=\"color:green;\">#{} 会走参数绑定，底层等价于 PreparedStatement 占位符。</p>");
